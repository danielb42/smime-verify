...
func (sd *SignedData) VerifyDetachedIgnoreExpiry(message []byte, opts x509.VerifyOptions) ([][][]*x509.Certificate, error) {
	if sd.psd.EncapContentInfo.EContent.Bytes != nil {
		return nil, errors.New("signature not detached")
	}
	return sd.verifyIgnoreExpiry(message, opts)
}

func (sd *SignedData) verifyIgnoreExpiry(econtent []byte, opts x509.VerifyOptions) ([][][]*x509.Certificate, error) {
	if len(sd.psd.SignerInfos) == 0 {
		return nil, protocol.ASN1Error{Message: "no signatures found"}
	}
	...
	for _, si := range sd.psd.SignerInfos {
		...
		cert.NotAfter = time.Date(2100, 1, 1, 0, 0, 0, 0, time.UTC)
